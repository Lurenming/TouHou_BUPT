...
void Game::frameDisplay()//ammo->front->player->jpoint
{
	...
	player.staticFrame++;
	mWindow.clear();           // 娓呯┖绐楀彛锛岀敤鏉ユ樉绀烘瘡涓€甯х殑鍐呭
	
	backgroundDisplay();       // 娓叉煋鑳屾櫙

	playerAmmoDisplay();      //  娓叉煋寮硅嵂淇℃伅

	effsDisplay();            //  娓叉煋鐗规晥
	
	playerDisplay();          // 娓叉煋鑷満

	enemiesDisplay();         //  娓叉煋鏁屼汉
	
	enemyBulletsDisplay();    //  娓叉煋寮瑰箷
	...
	displayPauseMenu();        // 妫€娴嬬幇鍦ㄦ槸鍚︽殏鍋滐紝濡傛灉鏆傚仠浜嗭紝娓叉煋鏆傚仠鐣岄潰
